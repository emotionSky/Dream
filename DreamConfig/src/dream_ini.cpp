...
DreamIni* DreamIni::m_pIns = nullptr;
...
const char* DreamIni::GetString(const char* key)
{
	if (key)
	{
...
int DreamIni::GetInt(const char* key, int def)
{
	if (key)
	{
		const auto& data = m_pData->m_str;
...
void DreamIni::SetString(const char* key, const char* value)
{
	auto& data = m_pData->m_str;
	data[std::string(key)] = std::string(value);
}

void DreamIni::SetInt(const char* key, int value)
{
	auto& data = m_pData->m_str;
