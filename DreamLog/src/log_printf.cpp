...
static u_char *log_sprintf_num(u_char *buf, u_char *last, uint64_t ui64, u_char zero, uintptr_t hexadecimal, uintptr_t width);
...
u_char *log_slprintf(u_char *buf, u_char *last, const char *fmt, ...)
{
	...
	int ret = vsnprintf((char*)buf, (last - buf), fmt, args); ///<使用vsnprintf函数
	if (ret < 0)
	{
		va_end(args);                     ///<释放args
		return buf;
...
u_char *log_vslprintf(u_char *buf, u_char *last, const char *fmt, va_list args)
{
	...
	/** 需要逐个对字符串进行判断，是否已经到达了字符串的结尾（'\0'），以及是否到达了buf的尾地址 */
	while (*fmt && buf < last)
	{
		if (*fmt == '%') ///<%开头的一般都是需要被可变参数 取代的
		{
			/** -----------------变量初始化工作开始----------------- */
			zero = (u_char)((*++fmt == '0') ? '0' : ' '); ///<跳过 '%' ，然后得到%后面接的字符是否为0
			...
			/** 判断%后边是否是个数字，如果是个数字，就把这个数字取出来，比如%16，便能够提取出来后使得width=16 */
			while (*fmt >= '0' && *fmt <= '9')
			{
			...
			for (;;)
			{
				...
				case 'X':     ///<%X，X表示十六进制，并且十六进制中的A-F以大写字母显示，不要单独使用，一般是%Xd
					...
					sign = 0; ///<使用十六进制进行输出了，那么就必须以无符号形式进行输出
					fmt++;    ///<往后走一个字符
					continue; ///<回到for继续判断
...
///<Windows下暂时先不打印进程ID这样的数据
			...
			case 'f':                     ///<一般 用于显示double类型数据，如果要显示小数部分，则要形如 %.5f
				...
				///<正整数部分，先显示出来
				buf = log_sprintf_num(buf, last, ui64, zero, 0, width); ///<把一个数字 比如“1234567”弄到buffer中显示
				...
				if (frac_width) //指定了显示多少位小数
				{
					...
					buf = log_sprintf_num(buf, last, frac, '0', 0, frac_width); // frac这里是小数部分，显示出来，不够的，前边填充'0'字符
				}
				fmt++;
				continue; //重新从while开始执行
			...
			buf = log_sprintf_num(buf, last, ui64, zero, hex, width);
			fmt++;
		}
...
{
	//temp[21]
	u_char      *p, temp[LOG_INT64_LEN + 1];   //#define LOG_INT64_LEN   (sizeof("-9223372036854775808") - 1)     = 20   ，注意这里是sizeof是包括末尾的\0，不是strlen；             
	size_t      len;
	uint32_t    ui32;
	...
	p = temp + LOG_INT64_LEN; //LOG_INT64_LEN = 20,所以 p指向的是temp[20]那个位置，也就是数组最后一个元素位置

	if (hexadecimal == 0)  
	{
		if (ui64 <= (uint64_t) LOG_MAX_UINT32_VALUE)   //LOG_MAX_UINT32_VALUE :最大的32位无符号数：十进制是‭4294967295‬
		{
	...
	len = (temp + LOG_INT64_LEN) - p;  //得到这个数字的宽度，比如 “7654321”这个数字 ,len = 7
	...
	len = (temp + LOG_INT64_LEN) - p; //还原这个len，也就是要显示的数字的实际宽度【因为上边这个while循环改变了len的值】
	...
	return log_memcpy(buf, p, len); //把最新buf返回去；
}