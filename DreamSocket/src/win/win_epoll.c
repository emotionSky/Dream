...
#include "DreamSky/win/win_epoll.h"
...
#define X(return_type, attributes, name, parameters) \
  static return_type(attributes* name) parameters;
...
static int afd_create_device_handle(
	d_handle iocp_handle,
	d_handle* afd_device_handle_out);

static int afd_poll(
	d_handle afd_device_handle,
	AFD_POLL_INFO* poll_info,
	IO_STATUS_BLOCK* io_status_block);

static int afd_cancel_poll(
	d_handle afd_device_handle,
	IO_STATUS_BLOCK* io_status_block);
...
static int err_check_handle(d_handle handle);

#define IOCTL_AFD_POLL 0x00012024

// 这里不能使用类型强转，否则导致未知错误，原因暂不清楚
...
static int port_register_socket(
	port_state_t* port_state,
	sock_state_t* sock_state,
	sock_t socket);

static void port_unregister_socket(
	port_state_t* port_state,
	sock_state_t* sock_state);

static sock_state_t* port_find_socket(
	port_state_t* port_state,
	sock_t socket);

static void port_request_socket_update(
	port_state_t* port_state,
	sock_state_t* sock_state);

static void port_cancel_socket_update(
	port_state_t* port_state,
	sock_state_t* sock_state);

static void port_add_deleted_socket(
	port_state_t* port_state,
	sock_state_t* sock_state);

static void port_remove_deleted_socket(
	port_state_t* port_state,
	sock_state_t* sock_state);

static d_handle port_get_iocp_handle(port_state_t* port_state);
...
static void ts_tree_init(ts_tree_t* rtl);
static void ts_tree_node_init(ts_tree_node_t* node);

static int ts_tree_add(ts_tree_t* ts_tree,
	ts_tree_node_t* node,
	uintptr_t key);

static ts_tree_node_t* ts_tree_del_and_ref(ts_tree_t* ts_tree,
	uintptr_t key);
static ts_tree_node_t* ts_tree_find_and_ref(ts_tree_t* ts_tree,
	uintptr_t key);
...
static d_handle epoll__create(void)
{
	port_state_t* port_state;
	d_handle ephnd;
...
d_handle epoll_create(int size)
{
	if (size <= 0)
...
d_handle epoll_create1(int flags)
{
	if (flags != 0)
		return_set_error(NULL, ERROR_INVALID_PARAMETER);
...
int epoll_close(d_handle ephnd)
{
	ts_tree_node_t* tree_node;
	port_state_t* port_state;
...
int epoll_ctl(d_handle ephnd, int op, sock_t sock, struct epoll_event* ev)
{
	ts_tree_node_t* tree_node;
	port_state_t* port_state;
	int r;
...
err:
	...
	err_check_handle(ephnd);
	err_check_handle((d_handle)sock);
	return -1;
}
...
int err_check_handle(d_handle handle)
{
	DWORD flags;
...
static sock_t ws_get_base_socket(sock_t socket);

static bool init__done = false;
static INIT_ONCE init__once = INIT_ONCE_STATIC_INIT;

static BOOL CALLBACK init__once_callback(
	INIT_ONCE* once,
	void* parameter,
	void** context)
{
...
#define X(return_type, attributes, name, parameters) \
  static return_type(attributes* name) parameters = NULL;
NT_NTDLL_IMPORT_LIST(X)
#undef X
...
static poll_group_t* poll_group_acquire(port_state_t* port);
...
static d_handle poll_group_get_afd_device_handle(poll_group_t* poll_group);
...
{
	d_handle iocp_handle = port_get_iocp_handle(port_state);
...
d_handle poll_group_get_afd_device_handle(poll_group_t* poll_group)
{
...
static sock_state_t* sock_new(port_state_t* port_state, sock_t socket);
...
static int sock_set_event(
	port_state_t* port_state,
	sock_state_t* sock_state,
	const struct epoll_event* ev);

static int sock_update(
	port_state_t* port_state,
	sock_state_t* sock_state);
static int sock_feed_event(
	port_state_t* port_state,
	IO_STATUS_BLOCK* io_status_block,
	struct epoll_event* ev);
...
static inline d_handle port__create_iocp(void)
{
	d_handle iocp_handle =
...
port_state_t* port_new(d_handle* iocp_handle_out)
{
	port_state_t* port_state;
	d_handle iocp_handle;
...
{
	d_handle iocp_handle = port_state->iocp_handle;
...
static inline int port__ctl_add(
	port_state_t* port_state,
	sock_t sock,
	struct epoll_event* ev)
{
...
static inline int port__ctl_mod(
	port_state_t* port_state,
	sock_t sock,
	struct epoll_event* ev)
{
...
static inline int port__ctl_del(port_state_t* port_state, sock_t sock)
{
	sock_state_t* sock_state = port_find_socket(port_state, sock);
	if (sock_state == NULL)
		return -1;
...
static inline int port__ctl_op(
	port_state_t* port_state,
	int op,
	sock_t sock,
	struct epoll_event* ev)
...
sock_state_t* port_find_socket(port_state_t* port_state, sock_t socket)
{
...
void port_request_socket_update(
	port_state_t* port_state,
	sock_state_t* sock_state)
{
...
void port_cancel_socket_update(
	port_state_t* port_state,
	sock_state_t* sock_state)
{
...
void port_remove_deleted_socket(
	port_state_t* port_state,
	sock_state_t* sock_state)
{
	unused_var(port_state);
...
d_handle port_get_iocp_handle(port_state_t* port_state)
{
	assert(port_state->iocp_handle != NULL);
...
static d_handle reflock__keyed_event = NULL;
...
sock_state_t* sock_new(port_state_t* port_state, sock_t socket)
{
	sock_t base_socket;
	poll_group_t* poll_group;
	sock_state_t* sock_state;
...
int sock_update(port_state_t* port_state, sock_state_t* sock_state)
{
	...
	else if (sock_state->poll_status == SOCK__POLL_IDLE)
	{
		...
		sock_state->poll_info.Handles[0].Handle = (d_handle)sock_state->base_socket;
...
static inline ts_tree_node_t* ts_tree__find_node(
	ts_tree_t* ts_tree,
	uintptr_t key)
{
...
static inline sock_t ws__ioctl_get_bsp_socket(sock_t socket, DWORD ioctl)
{
	sock_t bsp_socket;
	DWORD bytes;
...
sock_t ws_get_base_socket(sock_t socket)
{
	sock_t base_socket;
	DWORD error;
